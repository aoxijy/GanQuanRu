...
# 添加第三方软件包
git clone https://github.com/aoxijy/dbone-packages.git -b 18.06 package/dbone-packages
...
#
sed -i 's#192.168.9.222#10.0.0.1#g' $NET                                                    # 定制默认IP
sed -i 's#GanQuanRu#g' $NET                                                     # 修改默认名称为OpenWrt-X86
sed -i 's@.*CYXluq4wUazHjmCDBCqXF*@#&@g' $ZZZ                                             # 取消系统默认密码
sed -i "s/OpenWrt /ONE build $(TZ=UTC-8 date "+%Y.%m.%d") @ GanQuanRu /g" $ZZZ              # 增加自己个性名称
